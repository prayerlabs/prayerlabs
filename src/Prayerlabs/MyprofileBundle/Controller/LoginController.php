    ...
    public function validatorAction(Request $request)
    {
    	...
    	if($email && $password)
		{
			...
			$query = $em->createQuery(
				'SELECT a FROM PrayerlabsMyprofileBundle:Accounts a WHERE a.email = :email and a.password = :password'
				)->setParameter('email', $email)->setParameter('password', md5('XvMpa12!'.$password));
